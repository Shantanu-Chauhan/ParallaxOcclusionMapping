...
{
	float start = glutGet((GLenum)GLUT_ELAPSED_TIME);
   ...
   // Draw all objects (hierarchically down through each object's child list).
	...
	int delta_time = glutGet((GLenum)GLUT_ELAPSED_TIME) - time_since_last_refresh;
	float speed = 0.01f;
	...
	vec3 Teapot(0.2, 0.0, 1.5);
	vec3 Light(3.5, 3.5, 3.5), Ambient(0.2, 0.2, 0.2);//:light and ambient set
	...
	int programId = shadowProgram->programId;
	...
	//shadow
	int loc = glGetUniformLocation(programId, "LightProj");
	...
	float end = glutGet((GLenum)GLUT_ELAPSED_TIME)-start;
}
